...
// inode bitmap
constexpr u32 INODE_BITMAP_SIZE = (TOTAL_INODES + BITS_PER_BYTE - 1) / BITS_PER_BYTE;
constexpr u32 INODE_BITMAP_SECTORS = (INODE_BITMAP_SIZE + BLOCK_SIZE - 1) / BLOCK_SIZE;
// block bitmap
constexpr u32 BLOCK_BITMAP_SIZE = (TOTAL_BLOCKS + BITS_PER_BYTE - 1) / BITS_PER_BYTE;
constexpr u32 BLOCK_BITMAP_SECTORS = (BLOCK_BITMAP_SIZE + BLOCK_SIZE - 1) / BLOCK_SIZE;
// inode table sectors
...
static_assert(INODES_PER_SECTOR > 0, "Inode is too big to fit in a 512-byte sector");
// *** BUG FIX: this used to be (TOTAL_INODES * INODE_SIZE + BLOCK_SIZE - 1) / BLOCK_SIZE,
// which assumes inodes pack tightly across sector boundaries with no waste.
// They don't -- InodeManager addresses inodes as (inode_number / INODES_PER_SECTOR),
// and INODES_PER_SECTOR floors to 7 here (512/72), wasting 56 bytes at the end of
// every sector instead of spilling into the next one. The old formula therefore
// reserved too few sectors (144 instead of the 147 actually needed for 1024
// inodes at 7-per-sector), so high inode numbers silently wrote into what the
// block allocator thought was free data space. Computing it from
// INODES_PER_SECTOR instead matches the real addressing scheme. ***
constexpr u32 INODE_TABLE_SECTORS =
	(TOTAL_INODES + INODES_PER_SECTOR - 1) / INODES_PER_SECTOR;
// ----- Disk layout (NO hard-coded 7 anymore) -----

...
public:
	...
	bool find_entry(u32 directory_inode, const char *name, DirectoryEntry &out) {
		...
		if (directory_inode >= TOTAL_INODES) return false;

		Inode dir_inode{};
		if (!inode_manager.read_inode(directory_inode, dir_inode)) return false;
		if (!dir_inode.used || !dir_inode.is_directory) return false;
		...
		for (u32 i = 0; i < DIRECT_BLOCKS; i++) {
			...
			if (block == INVALID_BLOCK) continue;
	...
	bool find_name_by_inode(u32 directory_inode, u32 child_inode, char out_name[MAX_FILENAME_LENGTH]) {
	...
	bool add_entry(u32 directory_inode, char *name, u32 inode_number) {
		if (!name) return false;
		if (directory_inode >= TOTAL_INODES) return false;
		if (inode_number >= TOTAL_INODES) return false;

		// must fit inside DirectoryEntry.name INCLUDING null terminator
		if ((u32)StringUtils::strlen(name) >= MAX_FILENAME_LENGTH) return false;
		...
		if (find_entry(directory_inode, name, existing)) return false;

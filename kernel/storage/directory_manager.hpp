...
public:
  ...
  bool add_entry(u32 directory_inode, char *name, u32 inode_number) {
    ...
    for (u32 i = 0; i < DIRECT_BLOCKS; i++) {
      ...
      DirectoryEntry *entries = reinterpret_cast<DirectoryEntry *>(buffers[i]);
    ...
    if (first_free != nullptr) {
      ...
      return block_manager.write_block(first_free_block, first_free_buffer);
    }
    ...
    return inode_manager.write_inode(directory_inode, dir_inode);
  }
    ...
    for (u32 i = 0; i < DIRECT_BLOCKS; i++) {
      ...
      for (u32 j = 0; j < DIRECTORY_ENTRIES_PER_BLOCK; j++) {
        ...
        if (StringUtils::strcmp(entries[j].name, name) == 0) {
          ...
          return block_manager.write_block(block, buffer);
        }
      }
    }
  ...
  bool find_name_by_inode(u32 directory_inode, u32 target_inode,
                          char *out_name) {
    Inode inode;

    if (!inode_manager.read_inode(directory_inode, inode))
      return false;
    ...
    for (int i = 0; i < DIRECT_BLOCKS; i++) {
      ...
      for (int j = 0; j < DIRECTORY_ENTRIES_PER_BLOCK; j++) {
        ...
        if (entries[j].inode_number == target_inode) {

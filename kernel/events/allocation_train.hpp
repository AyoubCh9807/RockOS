...
class Events {
  ...
  int count = 0;
   ...
   AllocationEvent allocation_train[MAX_ALLOCATION_EVENTS];

  public:

  bool is_event_pending() { return count != 0; }

  AllocationEvent consume_next_event() {
    if (count == 0)
      return AllocationEvent(AllocationEvent::FREED, 0);
  ...
  bool push_event(AllocationEvent ev) {
